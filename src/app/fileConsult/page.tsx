...
import {  FileCons } from "../types/destinations"
import {  fileAndConsultancy } from "@/sanity/lib/queries"
...
const FileList =()=>{
  const [destination, setDestination] = useState<FileCons[]>([])
  ...
  useEffect(()=>{
    ...
    if (token) {
      ...
      async function fetchedDestination(){
          const fetchedDestination: FileCons[] = await client.fetch(fileAndConsultancy)
        ...
        <div className="flex grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mb-8">
            ...
            {destination.map ((destination)=>(
                <div key={destination._id} className="sm:w-[290px] w-[150px] border-2  border-gray-300  rounded-xl bg-gray-100 xl:mx-10 mx-2 sm:mx-4 hover:shadow-md hover:shadow-black text-center my-3">
                 <Link href={`/fileConsult/${destination._id}`}>
...
<h3 className=" sm:text-md text-sm text-left flex ml-4 gap-1 font-bold sm:gap-2 text-lg text-black"><IoLocationOutline className="mt-1" />{destination.countryName}</h3>
                    <h2 className="text-left text-sm ml-6 sm:text-md mt-1">Rs. {isApprovedUser ? destination.prizeForUsers : destination.prize}</h2>
              
                    <button className="bg-orange-500 rounded-xl sm:w-[130px] w-[100px] text-white text-sm sm:text-md mb-4 mt-4 hover:bg-blue-500 hover:shadow-[0_4px_14px_rgba(0,0,0,0.2)] transition duration-300 shadow-lg font-serif">View Details</button></Link>
