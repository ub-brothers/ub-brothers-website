...
import {  HajjType } from "../types/destinations"
import {  hajj } from "@/sanity/lib/queries"
import Link from "next/link"
import { IoLocationOutline } from "react-icons/io5";

const HajjList =()=>{
  const [destination, setDestination] = useState<HajjType[]>([])
  

  useEffect(()=>{
      async function fetchedDestination(){
          const fetchedDestination: HajjType[] = await client.fetch(hajj)
  ...
  return (
    <div>
      ...
      <div>
        <h1 className="text-left xl:mx-10 sm:mx-4 mx-2 my-4 text-lg sm:text-3xl font-serif"><u>Hajj Packages 2025:</u></h1>
        <div className="flex grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mb-8">
            ...
            {destination.map ((destination)=>(
                <div key={destination._id} className="sm:w-[290px] w-[150px] border-2  border-gray-300  rounded-xl bg-gray-100 xl:mx-10 mx-2 sm:mx-4 hover:shadow-md hover:shadow-black text-center my-3">
                 <Link href={`/hajj/${destination._id}`}>
...
<h2 className="ml-4 my-2 text-left">{destination.dayDuration}</h2>
